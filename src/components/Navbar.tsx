...
const Navbar = () => {
  ...
  return (
    <div className={`fixed top-0 left-0 w-full z-50 transition-all duration-300 ${isScrolled ? "bg-gradient-to-r from-gray-900/95 to-slate-800/95 backdrop-blur-md shadow-lg" : "bg-transparent"}`}>
      <div className="container mx-auto flex items-center justify-between py-5 px-4 md:px-6">
        <div className="flex items-center">
          <a href="/" className="inline-flex items-center gap-3" aria-label="Home" onClick={handleLogoClick}>
            <img src="/lovable-uploads/3ecba068-9dac-495c-80c4-c1702aacc9a1.png" alt="VoiceSora Logo" className="h-9 w-auto" />
      ...
      <div 
        className={`md:hidden bg-gradient-to-b from-gray-900/95 to-slate-800/95 backdrop-blur-lg overflow-hidden transition-all duration-300 ease-in-out ${isMobileMenuOpen ? "max-h-64 opacity-100 border-t border-white/10" : "max-h-0 opacity-0"}`} 
          ...
          <nav className="flex flex-col space-y-4">
            {navLinks.map(link => (
              <a 
                key={link.name} 
                href={link.href} 
                className="text-base text-white/80 hover:text-white transition-colors py-3 font-medium focus-visible-ring border-b border-white/10 pb-3" 
                onClick={() => {
